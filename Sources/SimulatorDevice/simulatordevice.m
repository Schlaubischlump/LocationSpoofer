...
- (NSString * _Nullable)productName {
    // Make sure we can read the product family ID before we continue
    if (![_device respondsToSelector:@selector(deviceType)] ||
        ![[_device deviceType] respondsToSelector:@selector(productFamilyID)]) {
        return nil;
    }

    int productFamilyID = [[_device deviceType] productFamilyID];
    switch (productFamilyID) {
        case SimProductFamilyIPhone:
        case SimProductFamilyIPad:
            return @"iPhone OS";
        case SimProductFamilyAppleTV:
            return @"Apple TVOS";
        case SimProductFamilyAppleWatch:
            return @"Watch OS";
    }
    return nil;
}
...
- (NSString * _Nullable)productVersion {
    // Make sure we can read the product family ID before we continue
    if (![_device respondsToSelector:@selector(runtime)] ||
        ![[_device runtime] respondsToSelector:@selector(versionString)]) {
        return nil;
    }
    return [[_device runtime] versionString];
}
...
- (BOOL)resetLocation {
    ...
    if ([_device respondsToSelector:@selector(clearSimulatedLocationWithError:)]) {
        ...
        if (error != NULL) {
            LOG_ERROR("SimDevice \"%s\": Could not clear location.", self.udid.UTF8String);
        }

        return error == NULL;
    }

    LOG_FATAL("SimDevice \"%s\": Could not clear location. No viable method found.",  self.udid.UTF8String);
    return FALSE;
}
